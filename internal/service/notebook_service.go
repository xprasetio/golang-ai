...
func (c *notebookService) GetAll(ctx context.Context) ([]*dto.GetAllNotebooksResponse, error) {
	...
	notes, err := c.noteRepository.GetByIdNotebookIds(ctx, ids)
	if err != nil {
		return nil, err
	}
	for _, note := range notes {
		for _, res := range result {
			if res.Id == note.NotebookId {
				res.Notes = append(res.Notes, dto.GetAllNotebookResponseNote{
					Id:        note.Id,
					Title:     note.Title,
...
func (c *notebookService) Delete(ctx context.Context, id uuid.UUID) error {
	...
	err = noteRepo.DeleteByNotebookId(ctx, notebook.Id)
	if err != nil {
		return err
	}
