...
func (c *noteController) MoveNote(ctx *fiber.Ctx) error {
	...
	resp, err := c.noteService.MoveNote(ctx.Context(), &req)
	if err != nil {
		return err
	}

	return ctx.JSON(serverutils.SuccessResponse("Success Move Note", resp))
}
func (c *noteController) SemanticSearch(ctx *fiber.Ctx) error {
	q := ctx.Query("q", "")

	resp, err := c.noteService.SemanticSeach(ctx.Context(), q)
	if err != nil {
		return err
	}

	return ctx.JSON(serverutils.SuccessResponse("Success Semantic search", resp))
}
